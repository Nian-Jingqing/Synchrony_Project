...
cd D:\Dropbox\Projects\Emotional_Sharing_EEG\EEG_Data\LSL_November2021
list_of_files = dir('**/*.xdf');

for eeg_file = 20:size(list_of_files)
    
    path_to_file = [list_of_files(eeg_file).folder '\' list_of_files(eeg_file).name];
    path_new_folder = sprintf('D:\\Dropbox\\Projects\\Emotional_Sharing_EEG\\EEG_Data\\LSL_November2021\\%s_MOBI', list_of_files(eeg_file).name);
